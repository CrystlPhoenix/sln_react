  ...
  return (
    <div className="py-10 px-4">
      <div className="w-full text-center mb-10 inline-flex gap-2 items-center justify-center">
        <h1 className="text-xl md:text-2xl inline-block">
          WELCOME TO SLN ENGINEERING LIMITED
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-14 gap-10 md:gap-15">
        ...
        ><div className="bg-white rounded-md shadow-md p-4 mb-4">
            <h3 className="text-sm md:text-base ">
              Welcome to SLN Engineering Limited, a trusted provider of electrical
              and power engineering solutions. We deliver reliable, efficient, and
              safety-focused services backed by technical expertise and hands-on
              industry experience.
            </h3>

            <p className="text-sm py-4">
              We specialize in the design, installation, maintenance, and
              optimization of electrical power systems, supporting businesses with
              dependable solutions across operation, maintenance, and system
              performance.
            </p>
        </div>
          

          <ul className="mt-5 bg-white p-4 rounded-md shadow-md flex flex-col gap-2 text-sm md:text-base">
            <li className="flex items-center gap-2">
              <FaArrowRight className="bg-blue-800 text-white rounded-full p-1 w-5 h-5" />
        ...
        <motion.div
          ref={refTwo}
          className="grid grid-cols-1  sm:grid-cols-2 gap-5 two"
          initial={{ opacity: 0, x: 50 }}
          animate={isInViewTwo ? { opacity: 1, x: 0 } : {}}
          transition={{ duration: 0.8, ease: "easeOut" }}
        >
          {features.map((feature, index) => (
            <div className="bg-white rounded-md shadow-md p-3" key={index}>
              <div className=" flex items-center justify-between mb-3 relative flex-col-reverse lg:flex-row gap-4">
                <h1 className="font-semibold z-10">{feature.title}</h1>
                <h1 className="text-5xl text-blue-200 font-semibold static lg:absolute z-[0] left-[-4%]">
                  {feature.number}
                </h1>
                <img src={feature.image} alt="" className="w-[3rem]" />
              </div>
